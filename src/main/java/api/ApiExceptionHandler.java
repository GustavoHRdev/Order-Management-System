...
public class ApiExceptionHandler {
    ...
    public org.springframework.http.ResponseEntity<ApiErrorResponse> handleBusinessExceptions(RuntimeException exception,
                                                                                               HttpServletRequest request) {
        HttpStatus status = NOT_FOUND_MESSAGES.contains(exception.getMessage()) ? HttpStatus.NOT_FOUND : HttpStatus.BAD_REQUEST;
        return buildResponse(status, exception.getMessage(), request, new HttpHeaders());
    }
